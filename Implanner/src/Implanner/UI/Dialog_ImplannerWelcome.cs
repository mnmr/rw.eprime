...
namespace Implanner.UI
{
    ...
    /// one-line description, and a framed pointer to the toolbar button
    /// that opens the main window. Close and "Take me there now" both
    /// dismiss it; the latter also opens the main window. ESC closes it via
    /// the standard cancel path while it has focus.
    public class Dialog_ImplannerWelcome : Window
    {
        ...
        // The preview art's vertical ink band in the 640x360 SVG (see
        // assets/preview.svg: illustration y 70.6..299.4), padded by the
        // same 16 logical units the conveyor keeps to the left edge. Drawn
        // via texcoords so the file's empty top and bottom never render.
        private const float CropTop = 54.6f / 360f;
        private const float CropBottom = 315.4f / 360f;
        private static readonly Rect PreviewTexCoords = new Rect(
            0f, 1f - CropBottom, 1f, CropBottom - CropTop);
        private const float PreviewAspect = 640f / (360f * (CropBottom - CropTop));
        ...
        public Dialog_ImplannerWelcome()
        {
            forcePause = true;
        ...
        {
            title = "IMP_WelcomeTitle".Translate();
            body = "IMP_WelcomeBody".Translate();
            find = "IMP_WelcomeFind".Translate();
            takeMeThere = "IMP_WelcomeTakeMeThere".Translate();
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            using (GuiStateScope.Capture())
            {
                ...
                GUI.DrawTexture(new Rect(findRect.x + PanelPad,
                        ...
                        IconSize, IconSize),
                    Patches.ImplannerTex.ToolbarButton);
        ...
        private static Texture2D? LoadPreview()
        {
            string path = Path.Combine(
                Path.Combine(ImplannerMod.ContentRootDir, "About"),
                "Preview.png");
            try
            {
                ...
                {
                    texture.name = "ImplannerWelcomePreview";
                    return texture;
                }
