...
{
    /// The dialog's tab strip: vanilla TabDrawer/TabRecord.Draw geometry with
    ...
    internal static class PlannerTabs
    {
        ...
        internal static void DrawTabs(Rect baseRect, List<TabRecord> tabs)
        {
            if (tabs.Count == 0) return;
            float tabWidth = (baseRect.width + (tabs.Count - 1) * TabOverlap)
                / tabs.Count;
            ...
            try
            {
                ...
                for (int d = maxDist; d >= 1; d--)
                {
                    int left = anchor - d;
                    if (left >= 0)
                        DrawTab(TabRect(left, tabWidth), tabs[left], -1);
                    int right = anchor + d;
                    if (right < tabs.Count)
                        DrawTab(TabRect(right, tabWidth), tabs[right], 1);
                }
                DrawTab(TabRect(anchor, tabWidth), tabs[anchor], 0);
        ...
        {
            Texture2D atlas = ImplannerTex.TabAtlas;
