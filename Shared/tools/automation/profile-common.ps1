function Get-InstalledPackageIds {
    ...
    foreach ($folder in @('Readouts','WorkRoles','QualityJobs','Implanner','Shared\Automation')) {
        $about = Join-Path $ModSourceRoot ($folder + '\mod\About\About.xml')
        if (Test-Path -LiteralPath $about) {
            [xml]$meta = Get-Content -LiteralPath $about -Raw
            $ids.Add(([string]$meta.ModMetaData.packageId).Trim()) | Out-Null
        }
    }
    return ,$ids
}
...
function Build-ModSetList {
    param(
        ...
        [Parameter(Mandatory)][string]$ModSourceRoot
    )
    $installed = Get-InstalledPackageIds $ModSourceRoot
    ...
    foreach ($raw in Get-Content -LiteralPath $SetPath) {
        ...
        if (-not $installed.Contains($line)) {
            throw "mod set package '$line' is absent from installed content and selected build sources"
        }
        $already = $false
...
function Initialize-RunProfile {
    param([string]$Profile, [string]$SourceSaveDirectory, [string]$SavePattern, [string]$ModSet, [string]$ModSourceRoot)
...
$profileConfig = Join-Path $Profile 'Config'
$profileSaves = Join-Path $Profile 'Saves'
$profileCaptures = Join-Path $Profile 'Captures'
...
if ($prefs.PrefsData.volumeMaster -ne '0' -or
        $prefs.PrefsData.adaptiveTrainingEnabled -ne 'False' -or
...
$activeMods = @(Build-ModSetList -SaveModIds $saveModIds -SetPath (Join-Path $PSScriptRoot 'modsets\automation-runtime.txt') -ModSourceRoot $ModSourceRoot)
if ($ModSet) {
...
$state | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath `
    (Join-Path $Profile 'profile-state.json') -Encoding utf8

Write-Host "run profile refreshed from $($sourceSave.FullName)"
