...
namespace RimShared.Common
{
    ...
    public static class InvariantDefName
    {
        public static string Humanize(string? defName, string? prefix = null)
        {
            ...
            string text = defName!; // Guarded by IsNullOrWhiteSpace above (no NotNullWhen on netstandard2.0).
            if (!string.IsNullOrEmpty(prefix)
                && text.StartsWith(prefix, System.StringComparison.Ordinal))
                text = text.Substring(prefix!.Length); // Guarded by IsNullOrEmpty above (no NotNullWhen on netstandard2.0).
