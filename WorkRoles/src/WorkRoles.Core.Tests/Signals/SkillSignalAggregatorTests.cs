...
public class SkillSignalAggregatorTests
{
    ...
    public async Task EnabledSkillsStartNeutralAndPassiveOrGlobalSignalsDoNotContribute()
    {
        ...
        await Assert.That(RequiredSkill(result, "Cooking").Bucket).IsEqualTo(SignalBucket.Neutral);
        await Assert.That(RequiredSkill(result, "Cooking").Contributions.Count).IsEqualTo(0);
        await Assert.That(RequiredSkill(result, "Shooting").Bucket).IsEqualTo(SignalBucket.Neutral);
    }
    ...
    public async Task TwoUnopposedPoorContributionsBecomeAwful()
    {
        ...
        SkillBucketSignal twoPoor = RequiredSkill(
            SkillSignalAggregator.Aggregate(skills, new SignalSnapshot(signals)), "Shooting");
    ...
    public async Task StrongContributionOffsetsTwoPoorContributions()
    {
        ...
        SkillBucketSignal offset = RequiredSkill(
            SkillSignalAggregator.Aggregate(skills, new SignalSnapshot(signals)), "Shooting");
    ...
    public async Task ExplicitAwfulIsAHardVetoAgainstAnyBenefits()
    {
        ...
        SkillBucketSignal result = RequiredSkill(
            SkillSignalAggregator.Aggregate(skills, new SignalSnapshot(signals)), "Shooting");
    ...
    public async Task UnknownActiveSignalsRemainNeutralWithUnclassifiedProvenance()
    {
        ...
        SkillBucketSignal result = RequiredSkill(
            SkillSignalAggregator.Aggregate(skills, new SignalSnapshot(signals)), "Cooking");
    ...
    public async Task PawnSnapshotCachesSignalsAndTheirAggregatedSkillBucketsTogether()
    {
        ...
        await Assert.That(RequiredSkill(snapshot.SkillBuckets, "Cooking").Bucket).IsEqualTo(SignalBucket.Neutral);
        await Assert.That(RequiredSkill(snapshot.SkillBuckets, "Melee").Bucket).IsEqualTo(SignalBucket.Strong);
        await Assert.That(ReferenceEquals(RequiredSkill(snapshot.SkillBuckets, "Melee").Contributions[0].Signal, brawlerMelee)).IsTrue();
    }
    ...
    public async Task PawnSnapshotClassifiesWorkAversionAsAnExactAwfulWorkType()
    {
        ...
        WorkTypeBucketSignal cooking = snapshot.WorkTypeBuckets.ForWorkType("Cooking")
            ?? throw new InvalidOperationException("Cooking work-type bucket was not produced.");
    ...
    public async Task BestFitRanksAggregatedBucketBeforeSkillLevel()
    {
        ...
        SkillBucketChoice result = SkillBucketRanking.Best(buckets, [new SkillBucketCandidate("Cooking", 20), new SkillBucketCandidate("Crafting", 2), new SkillBucketCandidate("Mining", 12)])
            ?? throw new InvalidOperationException("Best-fit ranking returned no choice.");
    ...
    private static SkillBucketSignal RequiredSkill(SkillBucketSnapshot snapshot, string skillDefName) =>
        snapshot.ForSkill(skillDefName)
        ?? throw new InvalidOperationException($"No bucket was produced for {skillDefName}.");
