...
public class HelpFlowLayoutTests
{
    ...
    {
        var doc = HelpMarkdown.Parse("See [the tour](topic:start) now.");
    ...
    {
        var doc = HelpMarkdown.Parse("Press **{k:WR_MakeItSo}** or {k:WR_Missing}.");

        var resolved = HelpDocuments.ResolveKeyedLabels(doc,
            key => key == "WR_MakeItSo" ? "Make It So" : null);
