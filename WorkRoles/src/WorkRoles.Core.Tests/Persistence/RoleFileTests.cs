...
public class RoleFileTests
{
    ...
    public async Task ParserPopulatesAlignedLegacyAndRichCollections()
    {
        ...
        await Assert.That((parsed.trainingPaths[0].anchorWithId
            ?? throw new InvalidOperationException("Anchor reference was not parsed.")).fileId).IsEqualTo("role-a");
    ...
    public async Task StableIdsDisambiguateDuplicateLabelsAcrossReferences()
    {
        ...
        int[] resolvedOrder = parsed
            .recommendationOrderWithIds.Select(reference =>
            {
                FileRole role = RoleFile.ResolveRole(parsed, reference.fileId, reference.label)
                    ?? throw new InvalidOperationException("Recommendation role did not resolve.");
        ...
        await Assert.That(parsed.groupsWithIds.Select(group => group.fileId
            ?? throw new InvalidOperationException("Group identity was not parsed."))).IsEquivalentTo(["group-a", "group-b"]);
    ...
    public async Task MalformedAndMissingIdsFallBackToLabels()
    {
        ...
        FileRole beta = RoleFile.ResolveRole(parsed, "dup-role", "Beta")
            ?? throw new InvalidOperationException("Beta role did not resolve.");
        FileGroup second = RoleFile.ResolveGroup(parsed, beta.groupId, beta.group)
            ?? throw new InvalidOperationException("Second group did not resolve.");
        var resolved = RoleFile.ResolvePathEntries(
