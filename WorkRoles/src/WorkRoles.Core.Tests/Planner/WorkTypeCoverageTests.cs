...
public class WorkTypeCoverageTests
{
    ...
    public async Task MovedGiversDetectsMovesSkipsUnmovedMissingAndKnown()
    {
        ...
        Dictionary<string, List<string>> requiredMoved = moved
            ?? throw new InvalidOperationException("Moved coverage was not produced.");
        await Assert.That(string.Join(",", requiredMoved["Doctor"])).IsEqualTo("TendAnimals");
    }
